...
export default function ProfilesPage() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setFormData({ 
        ...
        notifications: {
          email: (profile as any)?.notifications?.email ?? true,
          push: (profile as any)?.notifications?.push ?? true,
          birthday: (profile as any)?.notifications?.birthday ?? true,
          tripUpdates: (profile as any)?.notifications?.tripUpdates ?? true
        }
      });
      setHasChanges(false);
    }
  ...
  useEffect(() => {
    if (profile) {
      ...
      const hasNotificationChanged = JSON.stringify(formData.notifications) !== JSON.stringify(currentNotifications);
      
      setHasChanges(hasNameChanged || hasBirthdayChanged || hasPhoneChanged || hasBioChanged || hasLocationChanged || hasCurrencyChanged || hasNotificationChanged);
    }
  }, [formData, profile]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      fd.append('notifications', JSON.stringify(formData.notifications));
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <Label className="text-sm font-medium text-gray-700">Thông báo sinh nhật</Label>
                        <p className="text-xs text-gray-500">Nhắc nhở sinh nhật thành viên</p>
                      </div>
                      <input
                        type="checkbox"
                        checked={formData.notifications.birthday}
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          notifications: { ...prev.notifications, birthday: e.target.checked }
                        }))}
                        className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                      />
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <Label className="text-sm font-medium text-gray-700">Cập nhật chuyến đi</Label>
                        <p className="text-xs text-gray-500">Thông báo khi có thay đổi chuyến đi</p>
                      </div>
                      <input
                        type="checkbox"
                        checked={formData.notifications.tripUpdates}
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          notifications: { ...prev.notifications, tripUpdates: e.target.checked }
                        }))}
                        className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                      />
                    </div>
